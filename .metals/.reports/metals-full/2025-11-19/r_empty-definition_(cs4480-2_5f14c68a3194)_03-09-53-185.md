...
// Run in Spark Shell with XGBoost jar:
...
import org.apache.spark.ml.evaluation.{BinaryClassificationEvaluator, Mu@@lticlassClassificationEvaluator}
...
try {
  println(s"📥 Loading preprocessed features from: $featuresPath")
  ...
  println("📊 Fixed Class distribution (label: 0=cat, 1=dog):")
  ...
  println(s"✅ Rows before cleaning: ${fixedDf.count()}")
  println(s"✅ Rows after cleaning:  ${clean.count()}")
  ...
  println(s"📚 Train size: ${train.count()}, Test size: ${test.count()}")
  ...
  def trainAndEval(model: Classifier, modelName: String): Unit = {
    ...
    println(s"🔎 $modelName predictions (sample):")
    ...
    println(f"📈 $modelName - Test AUC: $auc%1.4f, Accuracy: $acc%1.4f")

    // Confusion matrix
    println(s"🧮 $modelName Confusion Matrix:")
  ...
  // K-Nearest Neighbors (Exact, since data small: broadcast train)
  println("🚀 Training K-Nearest Neighbors (Exact, K=5)...")
  ...
  println(f"📈 KNN - Test AUC: $knnAuc%1.4f, Accuracy: $knnAcc%1.4f")
  ...
  println(s"🏆 Best model: $bestModelName")

  println("✅ ML training and comparison finished.")
